...
use crate::{
    ...
    mm::{translated_refmut, translated_str,VirtPageNum,VirtAddr,PhysAddr,
        MapPermission,frame_allocator::FRAME_ALLOCATOR},
    task::{
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    let Some(app_inode) = open_file(path.as_str(), OpenFlags::RDONLY) else { return -1; };
    let all_data = app_inode.read_all();
    let new_task_control_block = Arc::new(TaskControlBlock::new(&all_data.as_slice()));
